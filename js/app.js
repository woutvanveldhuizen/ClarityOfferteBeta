...
function clearItemForm(){['editIndex','itemKey','itemName','itemKeywords','itemMin','itemMax','itemMinPrice','itemMaxPrice'].forEach(id=>{const el=$('#'+id); if(el)el.value=''})}
function showItemForm(){ $('#itemForm').classList.remove('hidden'); renderTypeOptions(); setPlaceholders(); setTimeout(()=>$('#itemName').focus(),50); }
function openEditItem(i){const it=state.items[i]; if(!it)return; showItemForm(); $('#editIndex').value=i; $('#itemKey').value=it.id||''; $('#itemType').value=it.type||'area'; $('#itemName').value=it.nameRaw||itemName(it)||''; $('#itemKeywords').value=kwList(it).join(', '); $('#itemMin').value=it.min??''; $('#itemMax').value=it.max??''; $('#itemMinPrice').value=it.minPrice??it.price??''; $('#itemMaxPrice').value=it.maxPrice??it.price??'';}
...
function bind(){setTimeout(()=>$('#splash').classList.add('hide'),2000);if('serviceWorker'in navigator)navigator.serviceWorker.register('./service-worker.js').catch(()=>{});$$('nav button').forEach(btn=>btn.onclick=()=>{$$('nav button').forEach(b=>b.classList.remove('active'));btn.classList.add('active');$$('.page').forEach(p=>p.classList.remove('active'));$('#'+btn.dataset.page).classList.add('active');applyI18n()});$('#language').onchange=e=>{state.settings.lang=e.target.value;persist();applyI18n();fillSettings()};$('#themeSelect').onchange=e=>{state.settings.theme=e.target.value;document.body.classList.toggle('light',state.settings.theme==='light');persist()};$('#analyze').onclick=()=>{state.quote=analyzeText($('#jobText').value);persist();renderQuote()};$('#clearQuote').onclick=()=>{state.quote=[];$('#jobText').value='';persist();renderQuote()};$('#exportPdf').onclick=exportPdf;$('#addItem').onclick=()=>{clearItemForm();showItemForm()};$('#cancelItem').onclick=()=>{clearItemForm();$('#itemForm').classList.add('hidden')};$('#saveItem').onclick=saveItem;$('#addCustomer').onclick=()=>{clearCustomerForm();showCustomerForm()};$('#cancelCustomer').onclick=()=>{clearCustomerForm();$('#customerForm').classList.add('hidden')};$('#saveCustomer').onclick=saveCustomer;$('#customerSelect').onchange=e=>{const c=(state.customers||[]).find(x=>x.id===e.target.value);if(c)applyCustomerToQuote(c);else{state.settings.selectedCustomerId='';persist();}};$$('.help').forEach(b=>b.onclick=()=>alert(t(b.dataset.help)));['companyName','companyEmail','companyPhone','companyWeb','companyAddress','vatRate','validDays','customerName','customerStreet','customerPostcodeCity','customerCountry','customerEmail','customerPhone'].forEach(id=>{const el=$('#'+id); if(el)el.oninput=e=>{state.settings[id]=e.target.value; if(id.startsWith('customer'))state.settings.selectedCustomerId=''; persist(); if(id==='vatRate')renderQuote();}})}
function init(){document.body.classList.toggle('light',state.settings.theme==='light');bind();applyI18n();renderQuote();renderDb();renderCustomers();fillSettings();persist()}init();
