...
function normalizeValueRow(row) {
  ...
  return {
    value,
    minPrice: Number(row.minPrice ?? row.price ?? 0) || 0,
    maxPrice: Number(row.maxPrice ?? row.price ?? row.minPrice ?? 0) || 0
  };
}
  ...
  merged.forEach(item => {
    if (item.priceRows) {
      item.priceRows = item.priceRows
        ...
        .filter(Boolean)
        .sort((a, b) => {
          const ar = parseValueRange(a.value);
          const br = parseValueRange(b.value);
...
function priceRowForValue(it, value) {
  ...
  if (Number.isFinite(numberValue)) {
    const exact = rows.find(row => {
      const r = parseValueRange(row.value);
...
function itemValueLabel(it) {
  ...
  if (it.priceRows.length === 1) return it.priceRows[0].value + unitSuffix(it);
