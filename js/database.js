const LEXICON={
 ...
 base:{nl:'Basisobject',de:'Grundobjekt',pl:'Obiekt bazowy',kw:{nl:['basisobject'],de:['grundobjekt'],pl:['obiekt bazowy']}}
};
const TOKEN_MAP={
...
{id:'garage_detached',type:'extra',nameKey:'detached garage',keywordKeys:['detached','garage'],price:400},
...
function lexLabel(key,lang=state.settings.lang){return LEXICON[key]?.[lang]||key}
function tokenKeysFromText(text){return String(text||'').toLowerCase().split(/[\s,;]+/).map(w=>w.trim()).filter(Boolean).map(w=>TOKEN_MAP[w]||w)}
function translatePhraseKeys(keys,lang=state.settings.lang){return (keys||[]).map(k=>lexLabel(k,lang)).join(' ')}
...
function kwList(it,lang=state.settings.lang){
 const out=[];
