...
function renderQuote() {
  ...
  (state.quote || []).forEach((line, index) => {
    ...
    row.className = 'line quoteLineCompact';
    row.innerHTML = `
      <button class="quoteLineText" data-quote-edit="${index}" type="button">
        <b>${escapeHtml(quoteLineTitle(line))}</b>
        <small>${escapeHtml(quoteLineDescription(line))}</small>
      </button>
      <div class="quoteLinePrice">
        <b>${fmtMoney((line.qty || 1) * line.price)}</b>
        <button data-del="${index}" aria-label="Delete">×</button>
      </div>
    `;
    box.appendChild(row);
  });
  ...
  box.querySelectorAll('[data-del]').forEach(button => {
    button.onclick = () => {
      state.quote.splice(Number(button.dataset.del), 1);
      persist();
      renderQuote();
      renderManualList();
    };
...
function renderPriceRows(rows = []) {
  ...
  box.innerHTML = '';
  (rows.length ? rows : [{ value: '', minPrice: '', maxPrice: '' }]).forEach(row => addPriceRow(row));
}

function addPriceRow(row = { value: '', minPrice: '', maxPrice: '' }) {
  const box = $('#priceRows');
  if (!box) return;
  ...
  el.innerHTML = `
    <input class="rowValue" placeholder="10-20" value="${escapeHtml(row.value ?? '')}">
    <input class="rowMinPrice" type="number" inputmode="decimal" placeholder="600" value="${escapeHtml(row.minPrice ?? '')}">
    <input class="rowMaxPrice" type="number" inputmode="decimal" placeholder="900" value="${escapeHtml(row.maxPrice ?? '')}">
...
function readPriceRows() {
  return $$('#priceRows .priceRow')
    .map(row => ({
      value: row.querySelector('.rowValue').value.trim(),
      minPrice: Number(row.querySelector('.rowMinPrice').value) || 0,
      maxPrice: Number(row.querySelector('.rowMaxPrice').value) || 0
    }))
...
function bind() {
  ...
  $('#addPriceRow').onclick = () => addPriceRow();
