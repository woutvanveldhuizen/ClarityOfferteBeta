...
function ensureBusinessDefaults() {
  const c = state.customers || [];
  if (!c.some(customer => customer.id === 'cust_silke_bernadi')) {
    c.unshift({
      id: 'cust_silke_bernadi',
      name: 'Silke Bernadi',
      email: '',
      phone: '',
      street: 'Engasse 2a',
      postcodeCity: '55296 Harxheim',
      country: 'Deutschland'
    });
    state.customers = c;
  }

  state.settings.companyName = 'Clarity Entrümpelung';
  state.settings.companyEmail = 'info@clarityentrumpelung.de';
  state.settings.companyPhone = '+49 172 5168515';
  state.settings.companyWeb = 'www.clarityentrumpelung.de';
  state.settings.companyAddress = 'Inhaber: Rafal Maciocha\nNeuweg 49\n55130 Mainz';
  state.settings.companyBank = 'Bankverbindung\nIBAN: DE05 5519 0000 0130 8270 25\nBIC: MVBMDE55\nVolksbank Darmstadt Mainz eG';
}
...
function updateResetQuoteVisibility() {
  const button = $('#resetQuote');
  if (!button) return;
  button.classList.toggle('hidden', activePageKey() !== 'quote');
}
...
function renderQuote() {
  ...
  (state.quote || []).forEach((line, index) => {
    ...
    row.innerHTML = `
      <button class="quoteLineCopy" type="button" data-quote-open="${index}">
      ...
      <button class="quoteLineRemove" type="button" data-quote-remove="${index}" aria-label="${escapeHtml(t('remove'))}">×</button>
    `;
    box.appendChild(row);
  });

  box.querySelectorAll('[data-quote-remove]').forEach(button => {
    button.onclick = event => {
      event.stopPropagation();
      removeQuoteLine(Number(button.dataset.quoteRemove));
    };
  });
...
function upsertManualQuote(item) {
  const matchedValue = manualValueFor(item);
  if (itemNeedsManualValue(item) && matchedValue == null) return false;
  const existing = (state.quote || []).findIndex(line => line.id === item.id && line.manual);
  const line = quoteLineFromItem(item, matchedValue ?? defaultValueForItem(item), true);
...
function renderManualList() {
  ...
  Object.keys(groups).forEach((type, index) => {
    ...
    groups[type].forEach(item => {
      ...
      const current = selected
        ? ((state.quote || []).find(line => line.id === item.id && line.manual)?.matchedValue ?? '')
        : '';
  ...
  box.querySelectorAll('[data-manual-check]').forEach(check => {
    check.onchange = () => {
      ...
      if (!item) return;
      const valueInput = box.querySelector(`[data-manual-value="${CSS.escape(item.id)}"]`);
      if (check.checked && itemNeedsManualValue(item) && valueInput && valueInput.value.trim() === '') {
        check.checked = false;
...
function sortPriceRowElements() {
  const box = $('#priceRows');
  if (!box) return;

  const rows = $$('#priceRows .priceRow');
  rows
    .sort((a, b) => {
      const av = Number(a.querySelector('.rowMinValue')?.value.replace(',', '.') || 0);
      const bv = Number(b.querySelector('.rowMinValue')?.value.replace(',', '.') || 0);
      return av - bv;
    })
...
function renderPriceRows(rows = []) {
  ...
  const sorted = [...(rows || [])].sort((a, b) => {
    const ar = rowValueParts(a);
    const br = rowValueParts(b);
    return Number(ar.minValue || 0) - Number(br.minValue || 0);
  });
  sorted.forEach(row => addPriceRow(row));
}

function addPriceRow(row = { minValue: '', maxValue: '', minPrice: '', maxPrice: '' }, mode = null) {
  const box = $('#priceRows');
  if (!box) return;

  const parts = rowValueParts(row);
  const isFixed = mode ? mode === 'fixed' : String(parts.minValue) === String(parts.maxValue);
  const minPrice = row.minPrice ?? '';
  const maxPrice = row.maxPrice ?? row.minPrice ?? '';
  const el = document.createElement('div');
  el.className = `priceRow ${isFixed ? 'fixedRow' : 'rangeRow'}`;
  el.innerHTML = `
    <input class="rowMinValue" type="number" inputmode="decimal" placeholder="${isFixed ? 'Aantal' : 'Laagste'}" value="${escapeHtml(parts.minValue)}">
    <input class="rowMaxValue" type="number" inputmode="decimal" placeholder="Hoogste" value="${escapeHtml(isFixed ? parts.minValue : parts.maxValue)}" ${isFixed ? 'tabindex="-1" aria-hidden="true"' : ''}>
    <input class="rowMinPrice" type="number" inputmode="decimal" placeholder="Prijs" value="${escapeHtml(minPrice)}">
    <input class="rowMaxPrice" type="number" inputmode="decimal" placeholder="Hoogste prijs" value="${escapeHtml(isFixed ? minPrice : maxPrice)}" ${isFixed ? 'tabindex="-1" aria-hidden="true"' : ''}>
    <button type="button" class="rowDelete" aria-label="Delete">×</button>
  `;

  const syncFixed = () => {
    if (!el.classList.contains('fixedRow')) return;
    el.querySelector('.rowMaxValue').value = el.querySelector('.rowMinValue').value;
    el.querySelector('.rowMaxPrice').value = el.querySelector('.rowMinPrice').value;
  };

  el.querySelector('.rowDelete').onclick = () => el.remove();
  el.querySelectorAll('input').forEach(input => {
    input.addEventListener('input', syncFixed);
    input.addEventListener('change', sortPriceRowElements);
    input.addEventListener('blur', sortPriceRowElements);
  });
  box.appendChild(el);
  syncFixed();
  sortPriceRowElements();
}

function addFixedPriceRow() {
  addPriceRow({ minValue: 1, maxValue: 1, minPrice: '', maxPrice: '' }, 'fixed');
  $('#priceRowChooser')?.classList.add('hidden');
}

function addRangePriceRow() {
  addPriceRow({ minValue: 10, maxValue: 20, minPrice: '', maxPrice: '' }, 'range');
  $('#priceRowChooser')?.classList.add('hidden');
}
...
function readPriceRows() {
  return $$('#priceRows .priceRow')
    .map(row => {
      const fixed = row.classList.contains('fixedRow');
      const minValueRaw = row.querySelector('.rowMinValue').value.trim();
      const maxValueRaw = fixed ? minValueRaw : row.querySelector('.rowMaxValue').value.trim();
      ...
      const maxPriceRaw = fixed ? String(minPrice) : row.querySelector('.rowMaxPrice').value;
...
function renderCustomers() {
  ...
  (state.customers || []).forEach((customer, index) => {
    ...
    row.innerHTML = `
      ...
      <div class="dbItemChevron" aria-hidden="true">›</div>
    `;
    row.onclick = () => openEditCustomer(index);
    box.appendChild(row);
  });
}
...
function toCsvRows(section, records) {
  return (records || []).map((record, index) => [
    csvEscape(section),
    csvEscape(index + 1),
    csvEscape(record.id || ''),
    csvEscape(record.type || ''),
    csvEscape(record.titleRaw || record.name || record.nameRaw || ''),
    csvEscape(record.descriptionRaw || record.description || ''),
    csvEscape((record.keywords || []).join ? record.keywords.join(', ') : record.keywords || ''),
    csvEscape(record.priceMode || ''),
    csvEscape(record.price ?? ''),
    csvEscape(JSON.stringify(record.priceRows || [])),
    csvEscape(JSON.stringify(record))
  ].join(';'));
}

async function exportCsvBackup() {
  const rows = [
    ['sectie','nr','id','type','titel_naam','beschrijving','trefwoorden','prijsmodel','prijs','prijstabel_json','volledige_data_json'].join(';'),
    ...toCsvRows('items', state.items),
    ...toCsvRows('customers', state.customers),
    ...toCsvRows('quote', state.quote),
    ['settings','','settings','','','','','','','',csvEscape(JSON.stringify(state.settings || {}))].join(';')
  ];

  const csv = '\ufeff' + rows.join('\n');
  const date = new Date().toISOString().slice(0, 10);
  const filename = `clarity-backup-${date}.csv`;
  ...
  try {
    ...
    // Fall back to normal download below.
  }
  ...
  // iOS Safari/PWA fallback: open the CSV in a new tab if download is ignored.
  setTimeout(() => {
...
function resetQuoteAndCustomer() {
  state.quote = [];
  state.settings.selectedCustomerId = '';
  state.settings.customerName = '';
  state.settings.customerEmail = '';
  state.settings.customerPhone = '';
  state.settings.customerStreet = '';
  state.settings.customerPostcodeCity = '';
  state.settings.customerCountry = '';
  const job = $('#jobText');
  if (job) job.value = '';
  persist();
  fillSettings();
...
function bind() {
  ...
  const resetQuote = $('#resetQuote');
  if (resetQuote) resetQuote.onclick = resetQuoteAndCustomer;
  const exportCsv = $('#exportCsv');
  if (exportCsv) exportCsv.onclick = exportCsvBackup;
  $('#addItem').onclick = () => {
    clearItemForm();
    showItemForm();
  };
